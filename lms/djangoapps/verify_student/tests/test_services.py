...
class TestReverificationService(ModuleStoreTestCase):
    ...
    def test_start_verification(self, checkpoint_name):
        """
        Test the 'start_verification' service method. Check that if a
        reverification checkpoint exists for a specific course then this method
        returns that checkpoint otherwise it creates that checkpoint.
        """
    ...
    def test_get_status(self):
        ...
        'course_id'.
        """
        checkpoint_location = u'i4x://{org}/{course}/edx-reverification-block/finalterm_uuid'.format(
            org=self.course_key.org, course=self.course_key.course
        )
        reverification_service = ReverificationService()
        self.assertIsNone(
            reverification_service.get_status(self.user.id, unicode(self.course_key), checkpoint_location)
        )
    ...
    def test_skip_verification(self):
        """
        Test adding skip reverification attempt for the user
        """
        checkpoint_location = u'i4x://{org}/{course}/edx-reverification-block/finalterm_uuid'.format(
            org=self.course_key.org, course=self.course_key.course
        )
        ...
        reverification_service.skip_verification(self.user.id, unicode(self.course_key), checkpoint_location)
        self.assertEqual(
    ...
    def test_get_attempts(self):
        ...
        course_id = unicode(self.course_key)
        self.assertEqual(
            reverification_service.get_attempts(self.user.id, course_id, checkpoint_location),
            0
        )
